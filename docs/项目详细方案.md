...
| 赛题用户期望 | 本产品回应（摘要） |
|--------------|-------------------|
| 快速了解就业市场对应届岗位的能力要求，并拆解具体能力要求 | 岗位导入与检索；**岗位画像**接口生成结构化要求（技能、证书、软素质等）；可结合知识库检索增强理解 |
| 清晰、准确分析学生就业能力与就业意愿 | **表单录入 + 简历上传** 双入口；输出结构化 **学生就业能力画像**、**完整度** 与 **竞争力** 评分及维度得分 |
| 给出可操作的生涯发展报告（方案 + 行动计划），扬长避短、精准匹配 | **四维人岗匹配**（基础要求、职业技能、职业素养、发展潜力）+ 差距与建议；**七章节职业报告**（含路径与短中期计划）；支持 **结构化编辑** 与 **PDF 一键导出** |
...
- 构建 **不少于 10 个** 目标岗位的 **结构化画像**（可随导入数据扩展至全量岗位）。  
- 画像维度覆盖赛题要求方向，包括但不限于：**专业技能**、**证书要求**、**创新能力**、**学习能力**、**抗压能力**、**沟通能力**、**实习/项目能力** 等（以接口返回字段与契约为准）。  
- 支持对指定岗位 **生成 / 缓存复用** 画像，便于批量演示与复现。
...
- 数据来源：**简历上传** 或 **表单自行录入**。  
- 输出：**结构化能力画像**、**完整度评分**、**竞争力评分** 及与赛题方向一致的维度刻画。  
- 大模型用于 **简历解析与信息抽取** 等环节（具体边界见本文 **第四篇、第五篇**，避免与「规则评分」混淆）。
...
- **编辑与导出**：支持 **手动编辑** 各章节并保存；支持 **PDF 导出与下载**。

#### 5.5 技术要求与指标（赛题）

- **大模型应用**：以 **Pi Coding Agent** 作为业务侧统一智能体入口，用于任务规划、工具编排及与 **RAG（qmd + pgvector）** 协同；岗位画像当前以 **规则基线** 为主并可与 Agent / 检索增强演进衔接。  
- **可操作性、可解释性**：匹配结果包含 **四维得分、加权总分、差距列表、改进建议与解释字段**。  
- **准确性**：依据仓库内 **30 条手工抽样** 评测（20 条匹配 + 10 条画像），**人岗匹配关键技能匹配准确率 80%**、**画像关键信息准确率 92%**，综合判定 **达到赛题阈值**（≥80% / >90%）。数据来源：`data/evaluation/match.manual-samples.jsonl`、`data/evaluation/profile.manual-samples.jsonl`，复算命令见本文 **第九篇**。  
- **实用性**：Vue 3 前端流程页（首页、画像、匹配、报告、职业路径、Agent 等）与 REST API 完整联调，适合课堂演示与答辩录屏。
...
- **数据层**：**PostgreSQL** 承载岗位、画像、学生、匹配、报告与导出记录；**pgvector** 承载知识库向量与混合检索；**Neo4j** 承载岗位路径图谱。  
- **智能层**：**Pi Coding Agent** 统一编排；**qmd** 文档结构化切分 + 索引脚本（`knowledge:index:*`）支撑本地知识库建设（对应企业要求提交的「本地知识库资料」）。  
- **工程质量**：统一 **Node 22.12+**；`npm run type-check` 作为合入基线；接口版本化 `/api/v1`。
...
4. **可演进架构**：Repository 适配器模式，关系库 / 向量 / 图谱分层清晰，便于从演示环境扩展到院校私有化部署。
...
| 能力 | 当前状态 | 说明 |
|------|-----------|------|
| 报告 **智能润色** | 规划中 | 可基于 Agent + 用户确认 diff 方式接入 |
...
*文档版本：与仓库实现状态对齐；后续迭代请同步更新本文档（含作品概要章）及独立 `docs/` 下若仍维护的摘要文件。*
...
- **业务闭环**：岗位数据进入系统 → 岗位画像 → 学生画像（表单/简历）→ 人岗匹配 → 职业报告（多版本）→ 编辑 → PDF 导出 → 职业路径可视化。  
- **技术闭环**：Monorepo 工程 + 契约优先 + PostgreSQL/pgvector/Neo4j + Pi Coding Agent 编排 + RAG 管线持续演进。  
...
- **实现**：`jobs` 模块；`POST /api/v1/jobs/import` 导入；`POST /api/v1/jobs/profile/generate` 生成结构化画像；画像字段与契约类型 `JobProfile` 等对齐。  
- **验收**：导入 ≥10 条岗位记录后批量生成画像；抽检画像字段与职位描述一致性。  
- **状态**：已落地（规则基线生成，见第四篇 4.2）。
...
- **实现**：同一 API 返回 `transition_routes`；种子数据由 `npm run career-path:sync`（仓库根目录，实际执行 `career-backend` 脚本）同步。  
- **验收**：枚举 5 个规范岗位，逐岗核对 ≥2 条 `transition_routes`。  
- **状态**：已落地。
...
- **实现**：`profile` 模块；`POST /api/v1/profile`、`POST /api/v1/profile/resume`；返回 `completeness_score`、`competitiveness_score`、`dimension_scores` 等。  
- **验收**：同一学生两种入口各跑一次，检查结构化字段与评分稳定合理。  
- **状态**：已落地。
...
- **实现**：`PATCH /api/v1/reports/{report_id}` 手动编辑；`POST/GET .../exports` + `GET .../download` PDF。  
- **规划**：润色与完整性检查见概要介绍「边界」说明。  
- **状态**：编辑 + PDF 已落地；润色 / 完整性检查 / Docx 未作为默认交付。

**（5）技术要求与指标**

- **大模型**：Pi Coding Agent + 业务工具 + RAG 管线；禁止并行维护独立 Chat Completions 业务主链路（工程约束）。  
- **可解释匹配**：四维打分 + 权重 + 说明。  
- **准确率**：见第九篇。  
- **实用性**：前端多页面联调完成。
...
- **jobs**：岗位导入、分页查询、岗位画像生成与缓存。  
- **profile**：学生画像创建（表单/简历）、列表查询。  
- **matching**：创建匹配、列表、详情、指纹缓存。  
- **report**：报告版本生成、查询、章节 PATCH、PDF 导出与下载。  
- **career-path**：按 `job_id` 查询规范角色、图节点边、晋升与换岗路径。  
- **knowledge**：知识库索引、混合检索、评测相关 API（见 OpenAPI）。  
- **agent**：任务创建、查询、对话别名等，对接 Pi SDK。

### 2.4 关键业务时序（摘要）

**岗位导入**：前端 multipart → `jobs.route` → `jobs.service` 解析清洗 → `repository.pg` 持久化 → 返回导入条数。
...
- 唯一业务模型入口：`/api/v1/agent/*` -> `Pi Coding Agent`。
...
- **归一化**：`canonical_role_key` / `canonical_role_title` 与 `job_id` 的映射关系在后端维护。  
- **同步脚本**：`npm run career-path:sync` 将路径种子写入 Neo4j。  
- **运行时**：`GET /api/v1/career-paths/jobs/{job_id}` 聚合图数据与路径列表；可选 `student_profile_id` 为路径附加 **技能缺口** 提示。
...
- **查询**：`GET /api/v1/jobs`，支持 `keyword`、`industry`、`offset`、`limit`，用于学生浏览「市场侧」岗位列表。  
- **画像生成**：`POST /api/v1/jobs/profile/generate`，请求体含 `job_id` 与可选 `force_regenerate`。  
- **缓存**：若未强制重算且已有版本，返回 `cached: true`，节省演示时间与费用。  
- **画像内容（逻辑维度）**：硬技能列表、证书建议、软技能 / 通用素质、技能权重、摘要文本、置信度等（以实际 API 响应为准）。  
- **当前策略**：**规则基线** 从职位描述抽取与归类，保证稳定；后续可接入 **LLM 重述 + RAG 证据引用** 提升丰富度与准确率。

### 4.2 岗位画像与赛题「不少于 10 个」

工程上 **不限于 10 个**：导入多少岗位即可生成多少画像。赛题「不少于 10 个」为最低要求；验收时任选 10 个岗位展示即可。

### 4.3 职业路径图谱 API

**路径**：`GET /api/v1/career-paths/jobs/{job_id}`
...
**字段**：`file` 必填；`target_role` 必填；`name` 可选；`parse_mode`：`strict | tolerant`。

**说明**：简历解析链路可能调用 **大模型或解析服务** 做实体抽取；`tolerant` 模式在格式不完美时尽量返回可用结构，避免演示失败。
...
| 智能润色 | 规划中 | 建议：Agent 生成修订稿 + 用户 diff 确认 + 写回章节 |
| 完整性检查 | 规划中 | 建议：Zod/OpenAPI 级章节非空校验、关键字段检查表、可读性提示 |

### 4.9 与 Agent 的协同边界

Agent 可通过工具读取 **匹配结果、检索命中、报告模板**，执行「规划—检索—总结」任务；**默认报告生成**仍以模板为主保证稳定性（与 **第二篇 §2.6** 中「Agent 与模型边界」所述报告生成策略一致）。演进路线是 **按章节逐步 LLM 化**，每次变更配套评测回归。
...
- **Pi Coding Agent**：业务运行时 **统一智能体入口**（`/api/v1/agent/*`），负责任务拆解、工具调用与结果汇总。  
- **qmd + pgvector**：文档切分、向量索引与混合检索，为 Agent 提供 **证据片段**。  
- **岗位画像 / 报告默认链路**：规则与模板为主；LLM 用于简历解析、Agent 总结等 **明确边界** 的能力，避免「全文黑盒生成」导致答辩不可控。

### 5.2 禁止双主链路的工程原因

若同时存在 **直连 Chat Completions** 与 **Pi Agent** 两套业务入口，会导致：提示词分叉、结果不可比、密钥与审计分散、类型契约难以统一。本仓库架构文档明确 **仅保留 Pi 作为业务模型主入口**；历史别名 `/api/v1/agent/chat` 映射到当前任务型 Agent，而非并行体系。

### 5.3 Agent 任务与工具（概念）

结合 OpenAPI 中 Agent 相关 schema，任务步骤可包含（示例枚举，以契约为准）：任务规划、上下文拉取、知识检索、匹配评价、报告生成、最终回答等。工具层应对接：`knowledge/search`、匹配创建/查询、报告读写等 **受控能力**，并记录 **步骤轨迹** 供审计与失败重试。
...
| 路径 | 功能模块 | 用户任务 |
|------|-----------|-----------|
| `/` | dashboard | 总览、入口导航、数据导入相关入口（以实际页面为准） |
| `/profile` | profile | 表单 / 简历创建学生画像 |
| `/matching` | matching | 选择画像与岗位，发起匹配，跳转报告或路径 |
| `/report` | report | 查看 / 编辑报告版本，导出 PDF |
| `/career-paths` | career-path | 查看岗位图谱与晋升、换岗路径 |
| `/agent` | agent | 智能体任务演示（若启用） |

### 6.4 推荐演示脚本（答辩友好）

1. 导入岗位文件 → 列表检索目标行业岗位。  
2. 对目标岗位「生成画像」→ 展示结构化要求。  
3. 录入学生信息或上传简历 → 展示完整度与竞争力。  
4. 创建匹配 → 展开四维得分与差距建议。  
5. 生成报告 → 浏览七章 → 现场改一句 PATCH 保存 → 导出 PDF。  
6. 打开职业路径页 → 展示晋升与换岗路径；若传 `student_profile_id` 展示缺口提示。
...
- 岗位：`/api/v1/jobs/import`、`/api/v1/jobs`、`/api/v1/jobs/profile/generate`  
- 画像：`/api/v1/profile`、`/api/v1/profile/resume`  
- 匹配：`/api/v1/matches`、`/api/v1/matches/{match_id}`  
- 报告：`/api/v1/reports` 系列、`/api/v1/report-exports/{export_id}/download`  
- 路径：`/api/v1/career-paths/jobs/{job_id}`  
...
| Career Path | `GET` | `/api/v1/career-paths/jobs/{job_id}` | 查询岗位图谱与路径规划 |
...
- `POST /api/v1/jobs/profile/generate`：单岗位画像生成或缓存命中返回。
...
- **Node.js**：`>=22.12.0 <23`（见根目录 `package.json` engines 与 `.nvmrc` / `.node-version`）。  
...
- 简历多种格式（PDF/DOCX 等，以实际支持为准）与乱码文件容错。  
...
6. 职业报告生成与导出：已完成（MVP 已支持生成/查看/结构化编辑、多版本与 PDF 导出，Docx 作为后续增强项）。
