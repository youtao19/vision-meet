...
- **实用性**：Vue 3 前端（首页、数据流水线、岗位画像、学生画像、匹配、报告、职业路径等）与 API 联调；**独立 Agent 页已移除**，智能体通过 API 与业务页间接演示。
...
- **业务域**：**`jobs-intelligence`**（`/api/v2` 流水线、规范角色、岗位事实、V2 路径）；**`ai`**（`/api/v2/ai`）。  
...
| 能力 | 当前状态 | 说明 |
|------|-----------|------|
| 报告 **智能润色** | **后端已提供** | `POST /api/v2/ai/polish`；前端按「修订预览 → 确认 → PATCH」接入 |
...
*文档版本：与仓库实现状态对齐（**2026-04-08**，含 `jobs-intelligence`、`/api/v2/ai`、前端流水线与岗位画像页等）；后续迭代请同步更新本文档（含作品概要章）及独立 `docs/` 下若仍维护的摘要文件。*
...
- **业务闭环**：岗位数据进入系统 →（可选）**归一化流水线** → **规范岗位画像** → 学生画像（表单/简历）→ 人岗匹配 → 职业报告（多版本）→ 编辑 →（可选）**AI 润色** → PDF 导出 → 职业路径可视化。  
...
- 报告生成策略：职业报告 **默认** 仍以模板生成器为主；润色等走 **`/api/v2/ai/polish`**。
...
| 智能润色 | **后端已支持** | `POST /api/v2/ai/polish`；建议前端：展示修订 diff → 用户确认 → `PATCH` 写回章节 |
...
| `/report` | report | 查看 / 编辑报告版本，导出 PDF（可逐步接入 `/api/v2/ai/polish` 润色） |
...
5. 生成报告 → 浏览七章 → 现场编辑保存 →（可选）调用 **`/api/v2/ai/polish`** → 导出 PDF。  
