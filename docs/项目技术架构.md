...
- `"/job-profiles"`：岗位画像功能页（`features/job-profiles/pages/JobProfilesPage.vue`）。
- `"/career-paths"`：路径图谱功能页（`features/career-path/pages/CareerPathPage.vue`）。
- `"/matching"`：匹配分析功能页（`features/matching/pages/MatchingPage.vue`）。
- `"/report"`：职业报告功能页（`features/report/pages/ReportPage.vue`）。

### 6.3 前端与后端通信

- API 基地址由 `VITE_API_BASE_URL` 配置，默认 `http://127.0.0.1:8000`。
- `src/shared/api/career-path.ts` 对接路径图谱。
- `src/shared/api/jobs.ts` 对接岗位查询。
- `src/shared/api/job-profiles.ts` 对接岗位画像。
- `src/shared/api/matching.ts` 对接匹配分析。
- `src/shared/api/profile.ts` 对接学生画像列表与创建。
- `src/shared/api/report.ts` 对接职业报告。
...
- `POST /api/v2/jobs/import`：导入岗位数据（csv/tsv/json/xls/xlsx）。
- `GET /api/v2/jobs`：分页/关键词/行业查询岗位。

2. `profile` 域（`src/modules/profile`）

- `GET /api/v2/profile`：查询学生画像列表。
- `POST /api/v2/profile`：创建学生画像并计算完整度、竞争力与维度得分。
...
- 兼容策略：旧 Agent 兼容入口已删除，统一走 `/api/v2/ai/*`。
- 报告生成策略：职业报告当前固定使用模板生成器，保证输出稳定、可复现、可编辑。
- 明确禁止：不得在 `shared/llm`、独立 smoke 脚本或 report 模块中重新引入海外模型直连运行链路，避免仓库再次出现“双主链路”。
...
sequenceDiagram
  ...
  FE->>RT: POST /api/v2/jobs/import (multipart file)
