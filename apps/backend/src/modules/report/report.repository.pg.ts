...
export function createPgReportRepository(pool: Pool): ReportRepository {
  ...
  async function ensureSchema(): Promise<void> {
    if (!schemaReady) {
      schemaReady = (async () => {
        ...
        // career_report_exports 是报告的子表，与报告共享 schema 初始化生命周期
        await pool.query(`
  ...
  async function createExportRecord(
    ...
    const result = await pool.query(
      `
        INSERT INTO career_report_exports (id, report_id, format, file_name, file_size_bytes, download_path)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING *
      `,
      [input.id, input.report_id, input.format, input.file_name, input.file_size_bytes, input.download_path],
    );
  ...
  async function listExportRecordsByReportId(reportId: number): Promise<ReportExportListResponse> {
    await ensureSchema();
    const result = await pool.query(
      `SELECT * FROM career_report_exports WHERE report_id = $1 ORDER BY created_at DESC`,
      [reportId],
    );
    return {
  ...
  async function getExportRecordById(exportId: number): Promise<CareerReportExportRecord | null> {
    await ensureSchema();
    const result = await pool.query(
      `SELECT * FROM career_report_exports WHERE id = $1 LIMIT 1`,
      [exportId],
    );
