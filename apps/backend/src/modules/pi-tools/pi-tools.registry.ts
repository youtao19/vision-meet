...
import { createGenerateJobPictureBookTool } from "./jobs/generate-job-comic.tool.js";
import type { PiToolFactoryContext } from "./pi-tool-context.js";

/**
 * 装配当前 AI 任务主链路工具。
 * 注意：简历生成、学生画像生成仍保留独立任务入口；岗位绘本已纳入主链路，由 Pi 自行决定是否调用。
 */
export function createCorePiTools(context: PiToolFactoryContext): ToolDefinition[] {
  ...
  if (context.dependencies.jobComicsService) {
    tools.push(
      createGenerateJobPictureBookTool({
...
export { createGenerateJobPictureBookTool as createGenerateJobComicTool } from "./jobs/generate-job-comic.tool.js";
