/**
 * 文件作用：通用 Pi Agent 会话运行器。
 * 设计边界：这里只封装会话初始化、prompt 发送、流式消息采集和超时控制；
 * 不包含任何业务逻辑，所有业务解析由调用方通过 parseResult 回调完成。
 * 依赖方向：被 pi-tools/ 和 ai/ 共同引用，不依赖任何业务模块。
 */
...
import { summarizeAssistantMessage } from "./pi-utils.js";
