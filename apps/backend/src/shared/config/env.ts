...
const rawEnvSchema = z.object({
  ...
  CODEX_APP_SERVER_COMMAND: z.string().trim().min(1).default("codex"),
  CODEX_APP_SERVER_MODEL: z.string().trim().min(1).optional(),
  CODEX_APP_SERVER_TIMEOUT_MS: z.coerce.number().int().min(30000).max(600000).default(300000),
  ...
  AGENT_THINKING_LEVEL: z
    ...
    .default("medium"),
  AGENT_REPORT_THINKING_LEVEL: z
    .enum(["off", "minimal", "low", "medium", "high", "xhigh"])
    .default("minimal"),
  AGENT_RESUME_TIMEOUT_MS: z.coerce.number().int().min(10000).max(300000).default(180000),
  AGENT_REPORT_TIMEOUT_MS: z.coerce.number().int().min(10000).max(300000).default(180000),
